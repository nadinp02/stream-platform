...
class EnviosPedidos extends Conexion
{
    public $id;
    public $usuario;
    public $nombre;
    ...
    public function __construct()
    {
        ...
        if ($_ENV["DEVELOPMENT"]) {
            parent::sql("
                CREATE TABLE IF NOT EXISTS `envios_pedidos` (
            ...
            // parent::sql("ALTER TABLE `envios_pedidos` ADD CONSTRAINT `fk_estado_pedidos_usuario` FOREIGN KEY (`usuario`) REFERENCES `usuarios`(`cod`) ON DELETE CASCADE ON UPDATE CASCADE");
        }
    }
    ...
    public function add($array)
    {
        try {
            ...
            $sql = "INSERT INTO envios_pedidos ($attr) VALUES ($values)";
            $db = parent::conPDO();
            $stmt = $db->prepare($sql);
            $stmt->execute($array);
            $row = $db->lastInsertId();
    ...
    public function edit($array, $sql_condition)
    {
        try {
            ...
            $sql = "UPDATE envios_pedidos SET $query WHERE $condition";
    ...
    {
        try {
            $sql  = "DELETE FROM envios_pedidos WHERE id=$id AND usuario='$user'";
            $stmt = parent::conPDO()->prepare($sql);
            return $stmt->execute();
        } catch (\Exception $e) {
            echo $e->getMessage();
    ...
    public function list($filter = [], $order = '', $limit = '', $single = false)
    {
        try {
            ...
            $sql = "SELECT * FROM `envios_pedidos` WHERE $filterSql ORDER BY $orderSql $limitSql";
            $stmt = parent::conPDO()->prepare($sql);
            $stmt->execute($filter);
