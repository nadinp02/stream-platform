...
class Carrito
{
    ...
    public function return()
    {
        if (!isset($_SESSION["carrito"])) {
            ...
            foreach ($_SESSION["carrito"] as $key => $item) {
                if ($item["tipo"] == 'pr') {
                    $producto = Productos::list(["filter" => ["cod = '" . $item["id"] . "'"]], $_SESSION['lang'], true);
    ...
    public function changePriceByPayment(array $payment)
    {
        ...
        $this->set("producto_cod", "Metodo de Pago");
        $this->set("tipo", "mp");
        $this->set("opciones", '');
        $this->add();
    }
    ...
    public function checkPaymentsLimits()
    {
        ...
        $returnData = (isset($_SESSION["perfil-ecommerce"]["data"]) && $_SESSION["perfil-ecommerce"]["data"]["mostrar_precios"] == 1) ? $returnData : true;
        return $returnData;
    }
    ...
    public function checkPriceOnPayments($pago)
    {
        ...
        if ($pago['data']["monto"] != 0) {
            $totalCarrito = $this->precioSinMetodoDeEnvio();
            $metodoEnvio = $this->checkEnvio();
            $precio = (($totalCarrito * $pago['data']["monto"]) / 100) + $totalCarrito;
            if ($metodoEnvio != '') $precio = $precio + $metodoEnvio['precio'];
        } else {
            foreach ($_SESSION["carrito"] as $cartItem) {
                if ($cartItem["promo"] != '') {
    ...
    public function addShipping($envioData)
    {
        try {
            $this->deleteOnCheck("Envio-Seleccion");
            $this->deleteOnCheck("Metodo-Pago");
            if ($envioData['data']['limite']) {
                if ($this->precioSinMetodoDePago() >= $envioData['data']['limite']) {
                    $envioData['data']["titulo"] = $envioData['data']["titulo"] . " ¡GRATIS COMPRA SUPERIOR A $" . $envioData['data']['limite'] . "!";
                    $envioData['data']["precio"] = 0;
                }
            }

            $this->set("id", "Envio-Seleccion");
            $this->set("cantidad", 1);
            $this->set("promo", '');
            $this->set("titulo", $envioData['data']["titulo"]);
            $this->set("precio", $envioData['data']["precio"]);
            $this->set("producto_cod", "Metodo de Envio");
            $this->set("tipo", "me");
            $this->set("opciones", '');
            $this->add();
            return true;
    ...
    public function addBill($factura)
    {
        try {
            $this->deleteOnCheck("factura");
            if ($factura == true && $_SESSION["perfil-ecommerce"]["data"]["recargo_factura"] > 0) {
                $totalPrice = $this->precioSinMetodoDeEnvio();
                $percentInDecimal = $_SESSION["perfil-ecommerce"]["data"]["recargo_factura"] / 100;
                $price = $percentInDecimal * $totalPrice;

                $this->set("id", "factura");
                $this->set("cantidad", 1);
                $this->set("promo", '');
                $this->set("titulo", "Impuestos y otros gastos");
                $this->set("precio", $price);
                $this->set("producto_cod", "factura");
                $this->set("opciones", '');
                $this->set("tipo", "FA");
                $this->add();
            }
