...
class Admin
{
    ...
    public function listSearch($search, $limit)
    {
        $search = trim($search);
    ...
    public function login_admin($refresh = false)
    {
        if ($refresh) {
            ...
            $sql = "SELECT usuarios.id, usuarios.email, usuarios.password, usuarios.admin, roles_admin.rol,roles.nombre as rol_name FROM usuarios JOIN roles_admin ON usuarios.id = roles_admin.admin LEFT JOIN roles ON roles.cod = roles_admin.rol WHERE email = '$email' AND password = '$password' AND usuarios.admin = 1";
        } else {
            $sql = "SELECT usuarios.id, usuarios.email, usuarios.password, usuarios.admin, roles_admin.rol,roles.nombre as rol_name FROM usuarios LEFT JOIN roles_admin ON usuarios.id = roles_admin.admin LEFT JOIN roles ON roles.cod = roles_admin.rol WHERE email = '{$this->email}' AND password = '{$this->password}' AND usuarios.admin = 1";
        }
        $admin = $this->con->sqlReturn($sql);
        ...
        if ($contar > 0) {
            while ($row = mysqli_fetch_assoc($admin)) {
                ...
                if (!empty($row["rol"])) {
                    $roles = Roles::listForMenu(["cod ='" . $row['rol'] . "'"], "", "");
                    $this->rol["permissions"] = $roles['data'];
                }
            }
            $_SESSION["admin"] = [
                "site" => PublicFunction::normalizar_link(TITULO),
                "id" => $this->id,
                "email" => $this->email,
                "password" => $this->password,
                "rol" => $this->rol,
    ...
    {
        session_destroy();
        PublicFunction::headerMove(URL_ADMIN . "/index.php");
    }
    public function logout()
    {
        $f = new PublicFunction();
        unset($_SESSION["admin"]);
        PublicFunction::headerMove(URL_ADMIN);
    }
    ...
    public function loginForm()
    {
        ...
        if (isset($_POST["login"])) {
            $admin->set("email", isset($_POST["email"]) ? PublicFunction::antihack_mysqli($_POST["email"]) : '');
            $admin->set("password", isset($_POST["password"]) ? hash('sha256', PublicFunction::antihack_mysqli($_POST["password"]) . SALT)  : '');
...
?>
    ...
    public function list($filter = [], $order, $limit)
    {
        ...
        $orderSql = ($order != '') ? $order : "ORDER BY usuarios.id ASC";
