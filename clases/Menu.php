...
class Menu extends Conexion
{
    ...
    public static function listByLanguage($admin)
    {
    ...
    public function add()
    {
        ...
        if ($this->area == "'admin'") {
            $roles = new Roles();
            $roles->addDevPermissions($this->area, $this->titulo, $this->link, $this->idioma, 'admin-role');
        }
        return $query;
    }
            ...
            if (isset($areaData)) {
                foreach ($areaData as $area) {
                    ...
                    if (!$exist) {
                        ...
                        $this->add();
                        var_dump(self::list(["link = '$link'", "titulo = '$titulo'"], $idioma, true));
                        $_SESSION["admin"]["rol"]["permissions"][0][] = self::list(["link = '$link'", "titulo = '$titulo'"], $idioma, true)[0];
                    }
                }
            }
        }
    ...
    {
        $ul_class = empty($padre_id) ? "navbar-nav ml-auto" : "sub-menu";

        $r = array_filter($this->total, function ($value) use ($padre_id) {
            return $value['padre'] == $padre_id;
        });
        if ($r) {
            echo "<ul class='$ul_class' id='gen-main-menu'>";
            foreach ($r as $value) {
                if ($value["area"] == $area) {
                    ...
                    $li_class = ($child) ? "menu-item active" : "menu-item";
                    $a_class = ($child) ? "" : "";
                ...
                echo "<li class='$li_class '>
                        <div class='sidebar-widget-list-left '>
                            <label for='for" . $id . "' class='fs-12 text-uppercase ml-20'>
                                <input onclick='checkRow(" . $id . ")' class='mr-10' id='for" . $id . "' name='permissions[id][]' value='" . $id . "' " . $checked . " type='checkbox'>" . $value['titulo'] . "
                            </label>
                            <input type='checkbox' id='create-" . $id . "' value='1' class='" . $none . " pull-right mr-20' " . $agregar  . "  name='permissions[$id][crear]'>
                            <input type='checkbox' id='edit-" . $id . "'  value='1' class='" . $none . " pull-right mr-30' " . $editar  . "  name='permissions[$id][editar]'>
                            <input type='checkbox' id='delete-" . $id . "' value='1' class='" . $none . " pull-right mr-30' " . $eliminar  . "  name='permissions[$id][eliminar]'>
                        </div>";
                echo "</li>";
    ...
    public function build_admin_nav($padre_id = "", $child = false)
    {
        ...
        $r = array_filter($_SESSION["admin"]["rol"]["permissions"], function ($value) use ($padre_id) {
            return $value['padre'] == $padre_id;
        });
        if ($r) {
    ...
    {
        $types = explode('@', $type);
        $data = Categorias::list(["area = '" . $types[0] . "'"], '', '', $_SESSION['lang']);
        ...
        if ($types[1] == "categorias") {
            ...
            foreach ($data as $value) {
                $opciones_ .= "<li class='nav__item'><a href='" . URL . "/productos/c/" . PublicFunction::normalizar_link($value['data']['titulo']) . "/" . $value['data']['cod'] . "' class='nav__item-link'> " . $value['data']['titulo'] . "</a></li>";
            }
            $opciones_ .= "</ul>";
        } else {
            $opciones_ .= "<ul class='dropdown-menu'>";
            foreach ($data as $value) {
                $link = URL . "/productos/c/" .  PublicFunction::normalizar_link($value['data']['titulo']) . "/" . $value['data']['cod'];
                ...
                if (!empty($value["subcategories"])) {
                    ...
                    foreach ($value["subcategories"] as $value_) {
                        $opciones_ .= "<li class='nav__item'><a href='" . $linkSub . "/" . PublicFunction::normalizar_link($value_['data']['titulo']) . "/" . $value_['data']['cod'] . "' class='nav__item-link'> " . $value_['data']['titulo'] . "</a></li>";
                    }
                    $opciones_ .= "</ul>";
                }
                $opciones_ .= "</li>";
    ...
    {
        $urlDelete = ($area == 'web') ? 'op=menu' : 'op=configuracion&accion=modificarTec';
        $r = array_filter($this->total, function ($value) use ($padre_id) {
            return $value['padre'] == $padre_id;
        });
                            ...
                            <div class="col text-right">
                                <div class="btn-group" role="group" aria-label="Basic example">
                                    ...
                                    <?php if ($_SESSION["admin"]["crud"]["eliminar"]) { ?>
                                        <button name="delete" class="btn-small btn btn-danger deleteConfirm mobilestylebuttondelete" href="<?= URL_ADMIN ?>/index.php?<?= $urlDelete ?>&borrar=<?= $row['id']  ?>&idioma=<?= $idiomaCheck ?>"><i class="fa fa-times"></i></button>
...
<?php
    ...
    public function menuOptions($data, $subcat = false)
    {
        ...
        foreach ($data as $categoria) {
            $link = strtolower(PublicFunction::normalizar_link($categoria['data']['area'])) . "/c/" . PublicFunction::normalizar_link($categoria['data']['titulo']) . "/" . $categoria['data']['cod'];
            ...
            if ($subcat) {
                if (!empty($categoria["subcategories"])) {
                    ...
                    foreach ($categoria['subcategories'] as $subcategoria) {
                        $linksub = $link . "/" . PublicFunction::normalizar_link($subcategoria['data']['titulo']) . "/" . $subcategoria['data']['cod'];
    ...
    {
        $categoryData = Categorias::listAreas('', '', '');
        $options = '';
    ...
    public static function updateByLink($title, $link, $lang, $link_to_search)
    {
        $old_row = self::list(["link = '$link_to_search'"], $lang, true)[0];
        $sql = "UPDATE `menu` SET titulo = '$title', link = '$link' WHERE `area`= 'admin' AND `id`= " . $old_row['id'];
        $query = parent::sql($sql);
        return !empty($query) ? true : false;
    }
}
