...
class PublicFunction
{
    ...
    public static function antihackMulti($array)
    {
        foreach ($array as $key => $value) {
            $data[$key] = $value;
            if (is_array($value)) {
                self::antihackMulti($value);
            } else {
                if ($value != strip_tags($value)) continue;
                $data[$key] = ($value != '') ? self::antihack_mysqli($value) : null;
            }
        }
        return $data;
    }
    ...
    public static function antihack_mysqli($str)
    {
        $con = new Conexion();
        $conexion = $con->con();
    ...
    public function getPermissions($pageSite, $permissions)
    {
        ...
        foreach ($permissions as $permission) {
            if ((isset($pageSite["op"]) && (strstr($permission["link"], "op=" . $pageSite["op"]))) || (isset($pageSite["area"]) && (strstr($permission["link"], "area=" . $pageSite["area"])))) {
    ...
    public static function headerMove($location)
    {
    ...
    public static function changeRootContents($old, $new)
    {
        $con = new Conexion();
    ...
    static public function normalizar_link($string)
    {
    ...
    static public function normalizar_titulo_imagenes($string)
    {
        $string = str_replace("á", "a", $string);
        $string = str_replace("Á", "A", $string);
        $string = str_replace("ä", "a", $string);
    ...
    static public function normalizar_meli($string)
    {
        $string = str_replace("á", "a", $string);
        $string = str_replace("Á", "A", $string);
        $string = str_replace("ä", "a", $string);
    ...
    public function localidadesApi($palabra = '')
    {
        $con = new Conexion();

        $sql = " SELECT DISTINCT `_provincias`.`nombre` as provincia,`_localidades`.`nombre` as localidad FROM  `_localidades`
                 INNER JOIN _provincias ON `_localidades`.`provincia_id` =  `_provincias`.`id`   
                 WHERE `_localidades`.`nombre` LIKE '%$palabra%'  
                 ORDER BY
                 CASE WHEN `_localidades`.`nombre` LIKE '%$palabra%' THEN `_provincias`.`nombre` END ASC
                 LIMIT 25";
        $stmt = $con->conPDO()->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll();
    }

    public function provinciaApi($palabra = '')
    {
        $con = new Conexion();

        $sql = " SELECT DISTINCT `_provincias`.`nombre`  FROM   _provincias  
                 WHERE `_provincias`.`nombre` LIKE '%$palabra%'  
                 ORDER BY
                 CASE WHEN `_provincias`.`nombre` LIKE '%$palabra%' THEN `_provincias`.`nombre` END ASC
                 LIMIT 25";
        $stmt = $con->conPDO()->prepare($sql);
        $stmt->execute();
        return $stmt->fetchAll();
    ...
    public function duplicateAllDB($newDB, $oldDB)
    {
        ...
        $tables = $con->sqlReturn($sqlShow);
        $arrayTable = [];
        $tableForInsert = [];
        #END

        #SELECCIONO UN ORDEN DE CARGA
        $orderPriority = ["idiomas", "area", "categorias", "subcategorias", "tercercategorias", "contenidos", "productos", "envios", "estados_pedidos", "pagos", "estado", "usuarios", "pedidos", "detalle_pedidos"];
        ...
        while ($table = mysqli_fetch_assoc($tables)) {
            ...
            $tables_ = $con->sqlReturn("SHOW CREATE TABLE `$table`");
