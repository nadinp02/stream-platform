...
class Categorias extends Conexion
{
    ...
    public function deleteForArea($area, $idioma)
    {
        ...
        foreach ($categoriasArea as $catItem) {
            ...
            if (!empty(Imagenes::list(["cod" => $cod, "idioma" => $idioma], "", "", true))) {
                Imagenes::deleteAll(["cod" => $cod, "idioma" => $idioma]);
            }
        }
    ...
    public static function list($filter = [], $order = '', $limit = '', $idioma, $single = false,  $images = true)
    {
        $array = array();
        ...
        $categorias = parent::sqlReturn($sql);
        if (!empty($categorias) && $categorias->num_rows) {
            while ($row = mysqli_fetch_assoc($categorias)) {
                $img = Imagenes::list(["cod" => $row['cod'], "idioma" => $idioma], 'imagenes.orden ASC', '');
                $sub = Subcategorias::list(["categoria='" . $row['cod'] . "'"], '', '', $idioma);
    ...
    public function listIfHave($db, $area = '', $idioma = '')
    {
        ...
        $array = array();
        $sql = " SELECT `categorias`.`titulo`,`categorias`.`cod`,`categorias`.`id`,`categorias`.`area`, count(`" . $db . "`.`categoria`) as cantidad FROM `" . $db . "`,`categorias` WHERE categorias.idioma = '$idioma'  AND  `categoria` = `categorias`.`cod` $area  $productos GROUP BY categoria ORDER BY `categorias`.`orden` ASC , `categorias`.`titulo` ASC ";

        $listIfHave = parent::sqlReturn($sql);
        if ($listIfHave) {
            while ($row = mysqli_fetch_assoc($listIfHave)) {
                $img = Imagenes::list(["cod" => $row['cod'], "idioma" => $idioma], 'imagenes.orden ASC', '');
                $sub = Subcategorias::listIfHave($db, $row["cod"], $idioma);
    ...
    public function listExcel($filter = [], $order = '', $limit = '', $idioma)
    {
        ...
        if ($categorias->num_rows) {
            while ($row = mysqli_fetch_assoc($categorias)) {
                $sub = Subcategorias::listExcel(["categoria='" . $row['cod'] . "'"], '', '', $idioma);
    ...
    public function listForManyCods($cods, $idioma)
    {
        ...
        if ($categorias->num_rows) {
            while ($row = mysqli_fetch_assoc($categorias)) {
                $sub = Subcategorias::list(["categoria='" . $row['cod'] . "'"], '', '', $row["idioma"]);
    ...
    public function listForDiscount()
    {
        ...
        if ($listDiscount) {
            while ($row = mysqli_fetch_assoc($listDiscount)) {
                $sub = Subcategorias::listForDiscount($row['cod']);
                $array[] = array("data" => $row, "subcategories" => $sub);
            }
            return $array;
        }
