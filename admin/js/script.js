...
$(".ckeditorTextarea").each(function () {
  CKEDITOR.replace(this, {
    ...
    filebrowserImageBrowseUrl:
      "ckfinder/ckfinder.html?type=Images&responseType=json",
    filebrowserFlashBrowseUrl:
      "ckfinder/ckfinder.html?type=Flash&responseType=json",
    ...
    filebrowserFlashUploadUrl:
      "ckfinder/core/connector/php/connector.php?command=QuickUpload&type=Flash&responseType=json",
  });
});
...
function agregar_input(div, name) {
  ...
  $("#" + div).append(
    '<div class="col-md-12 input-group" id="' +
    cod +
    '"><input onkeydown="return (event.keyCode!=13);" type="text" class="form-control mb-10 mr-10" name="' +
    name +
    "[" +
    cod +
    '][atributo]"><input id="tg' +
    cod +
    '" onkeydown="return (event.keyCode!=13);" type="text" class="form-control mb-10 mr-10" name="' +
    name +
    "[" +
    cod +
    '][valores]"></div>'
  ...
  $("#" + cod).append(
    ' <div class="input-group-addon"><a href="#" onclick="$(\'#' +
    cod +
    '\').remove()" class="btn btn-primary"> <i class="fas fa-minus"></i> </a> </div>'
  );
  $("#tg" + cod).tagify();
}
...
function agregar_atributo(div) {
  ...
  $("#" + div).append(
    ...
    cod +
    '"><input onkeydown="return (event.keyCode!=13);" type="text" class="form-control mb-10 mr-10" name="atributo[]"></div>'
  );
  $("#" + cod).append(
    ' <div class="input-group-addon"><a href="#" onclick="$(\'#' +
    cod +
  ...
  if (permisos == "true") {
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data["status"]) {
          successMessage(
            "Producto " + data["producto"] + " actualizado correctamente"
          );
        } else {
          errorMessage(
            "El producto " + data["producto"] + " no se ha actualizado"
          );
        }
      },
    });
...
function editAndSendStatus(url, codPedido, estadoPedido, enviar) {
  ...
  $.ajax({
    ...
    beforeSend: function () {
      ...
      $("#textS").append(
        "<p class='fs-18 mt-10'>EXCELENTE, ESTAMOS ENVIANDO UN EMAIL CON LA INFORMACION DEL ESTADO</p>"
      );
      $("#textS").append("</div>");
      $("#modalS").modal("toggle");
    },
...
function getSelectedValues(id) {
  let selectedOptions = "<br><h5 class='fs-16'>Opciones seleccionadas</h5>";
  $("#select-" + id)
    .val()
    .forEach((element) => {
      selectedOptions =
        selectedOptions +
        " <span class='green fs-14'>" +
        element +
        "</span><br>";
    });
  $("#div-" + id).html(selectedOptions);
}


function deleteImg(id, idioma, single = false) {

  swal({
    title: "¿ESTÁS SEGURO DE ELIMINAR ESTA IMAGEN?",
    text: "No podrás recuperar esta imagen, una vez borrada.",
    icon: "warning",
    buttons: ["Cancelar", true],
    dangerMode: true,
  }).then((willDelete) => {
    if (willDelete) {
      if (idioma.search('byCod')) {
        $.ajax({
          url: url_admin + "/api/images/deleteById.php",
          type: "POST",
          data: {
            id: id,
            idioma: idioma,
          ...
          success: (data) => {
            ...
            if (data["status"]) {
              $("#img-" + id).remove();
              successMessage(data["message"]);
              if (single) $("#img-row").append("<label class='col-md-7'>Imágen:<br /><input type='file' id='file' name='files[]' multiple='multiple' accept='image/*' /></label>");
            } else {
              warningMessage(data["message"]);
            }
          },
        ...
        $.ajax({
          url: url_admin + "/api/images/delete.php",
          type: "POST",
          data: {
            url: id,
          },
  ...
  if (input.files && input.files[0]) {
    for (let index = 0; index < input.files.length; index++) {
      ...
      reader.onload = function (e) {
        $('#preview-images').append('<span class="mx-2"><img src="' + e.target.result + '" width="220"/></span>');
      }
