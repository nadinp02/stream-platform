...
<section id="tabs" class="project-tab   mb-20 mt-40">
    <h4>Configuraciones técnicas</h4>
    <hr />
...
if (isset($_GET["borrar"]) && $_SESSION["admin"]["crud"]["eliminar"]) {
    ...
    $menu->set("id", $id);
    $menu->delete();
    $funciones->headerMove(URL_ADMIN . "/index.php?op=configuracion&accion=modificarTec&idioma=$idiomaGet");
}
?>
...
<script>
    ...
    function createBackup() {
        swal({
            title: "¿ESTÁS SEGURO DE CREAR UN BACKUP?",
            icon: "warning",
            buttons: ["Cancelar", true],
            dangerMode: true,
        }).then((result) => {
            if (result) {
                $.ajax({
                    url: "<?= URL_ADMIN ?>/api/backup/create.php",
                    type: 'GET',
                    beforeSend: function() {
