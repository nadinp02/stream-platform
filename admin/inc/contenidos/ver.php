                                            ...
                                            <tbody>
                                                ...
                                                if (is_array($contenidoData)) {
                                                    foreach ($contenidoData as $contenido) { ?>
                                                        <input type="text" style="position:absolute;left:-1000px;top:-1000px;" id="link-<?= $contenido['data']['cod'] ?>" value="<?= URL . $contenido['area']["data"]['url'] . $contenido['data']['area'] . "/" . $funciones->normalizar_link($contenido['data']['titulo']) . "/" . $contenido['data']['cod'] ?>">
                                                        <tr role="row" class="odd">
                                                            <td>
                                                                <?php
                                                                if ($_SESSION["admin"]["rol_name"] == "desarrollador") {
                                                                    $icon = ($contenido["data"]["bloquear"]) ? "fa fa-lock" : "fa fa-unlock";
                                                                    $cod = $contenido['data']['cod'];
                                                                    $area = $areaData['data']['cod'];
                                                                    $change = ($contenido["data"]["bloquear"]) ? 0 : 1;
                                                                    $link = URL_ADMIN . "/index.php?op=contenidos&accion=ver&area=$area&cod=$cod&bloquear=$change";
                                                                ?>
                                                                    <a href="<?= $link ?>"><i class="<?= $icon ?>"></i></a>
                                                                <?php
                                                                }
                                                                ?>
                                                                <img class="ml-1" src="<?= URL_ADMIN ?>/img/idiomas/<?= $contenido["data"]["idioma"] ?>.png" width="20" />
                                                            </td>
                                                            <td>
                                                            ...
                                                            <td class="text-right">
                                                                <div class="btn-group" role="group" aria-label="Basic example">
                                                                    <button onclick="copyLink('<?= $contenido['data']['cod'] ?>')" class="btn btn-warning " data-toggle="tooltip" data-placement="left" title="Copiar url del contenido"><i class="fa fa-link" aria-hidden="true"></i></button>
                                                                    <?php if ($_SESSION["admin"]["crud"]["editar"]) { ?>
                                                                        <a data-toggle="tooltip" class="btn btn-default" data-placement="left" title="Modificar" href="<?= URL_ADMIN . '/index.php?op=contenidos&accion=modificar&cod=' . $contenido['data']['cod'] . "&area=" . $areaData['data']['cod'] . "&idioma=" . $idioma ?>">
                                                                    ...
                                                                    <?php } ?>
                                                                    <?php
                                                                    if (($_SESSION["admin"]["rol_name"] == "desarrollador") || ($_SESSION["admin"]["crud"]["eliminar"] && !$contenido["data"]["bloquear"])) { ?>
                                                                        <a class="deleteConfirm btn btn-danger" data-toggle="tooltip" data-placement="left" title="Eliminar" href="<?= URL_ADMIN . '/index.php?op=contenidos&accion=ver&area=' . $areaData['data']['cod'] . '&borrar=' . $contenido['data']['cod'] . '&idioma=' . $idioma ?>">
                                                                            <div class="fonticon-wrap">
...
$bloquear = isset($_GET["bloquear"]) ? $funciones->antihack_mysqli($_GET["bloquear"]) : '';
...
if ($_SESSION["admin"]["rol_name"] == "desarrollador" && $bloquear) {
    $contenidos->edit(["bloquear" => $bloquear], ["cod = '$cod'"]);
    $funciones->headerMove(URL_ADMIN . "/index.php?op=contenidos&accion=ver&area=" . $areaData['data']['cod'] . "&idioma=" . $idioma);
}
?>
<script>
