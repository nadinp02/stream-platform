                                ...
                                if (isset($_FILES['excel']['name']) && $_FILES['excel']['name'] != "") {
                                    ...
                                    foreach ($sheet as $cellVal) {
                                        ...
                                        if (!empty($cellVal[4]) && strpos($cellVal[4], '@') !== false) {  // VALIDA QUE EXISTA UN EMAIL SINO NO AGREGA/EDITA
                                            ...
                                            $tipo = (trim(mb_strtolower($cellVal[15], 'UTF-8')) == 'mayorista') ? 0 : 1; //  1- Minorista / 0- Mayorista
                                            $descuento = str_replace('%', '', $cellVal[16]); // Elimina el simbolo "%" puesto en el exportar
                                            $estado = (trim(mb_strtolower($cellVal[17], 'UTF-8')) == 'activo') ? 1 : 0; // 1- Activo / 0- Desactivado
                                            ...
                                            if ($checkExist['data'] != null) {
                                                ...
                                                $usuarios->editSingle("calle", $cellVal[8]);
                                                $usuarios->editSingle("numero", $cellVal[9]);
                                                $usuarios->editSingle("piso", $cellVal[10]);
                                                $usuarios->editSingle("localidad", $cellVal[11]);
                                                $usuarios->editSingle("provincia", $cellVal[12]);
                                                $usuarios->editSingle("pais", $cellVal[13]);
                                                $usuarios->editSingle("postal", $cellVal[14]);
                                                ...
                                                $usuarios->set("calle", $cellVal[8]); // Direccion
                                                $usuarios->set("numero", $cellVal[9]); // Direccion
                                                $usuarios->set("piso", $cellVal[10]); // Direccion

                                                $usuarios->set("localidad", $cellVal[11]); //Localidad
                                                $usuarios->set("provincia", $cellVal[12]); // Provincia
                                                $usuarios->set("pais", $cellVal[13]); // Pais
                                                $usuarios->set("postal", $cellVal[14]); // Codigo Postal
