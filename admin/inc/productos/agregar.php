...
$optionsWithoutCat = $opciones->list($idiomaGet, ["ISNULL(`opciones`.`categoria`)", "`opciones`.area = 'productos'"], false, "");
    ...
    if (isset($idiomasInputPost) && !empty($idiomasInputPost)) {
        foreach ($idiomasInputPost as $idiomasInputItem) {
            foreach ($opciones as $key => $optionData) {
                if ($optionData == "-- Sin seleccionar --" || $optionData == NULL || !is_string($optionData)) continue;
                            ...
                            <div class="my-2 py-50 row">
                                <div class="col-12 order-1 order-sm-1">
                                    ...
                                    <div class="custom-file" style="margin-top: 5px;">
                                        <input name="files[]" onchange="filePreview(this)" class="custom-file-input" type="file" id="file" multiple="multiple" accept="image/*">
