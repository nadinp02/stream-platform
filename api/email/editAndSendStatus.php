...
if (!empty($codPedido)) {
    ...
    if ($enviarMail == 1) {
        ...
        $envioData = $pedido->getInfoPedido($pedidoData['detalle_envio']);
        $pagoData = $pedido->getInfoPedido($pedidoData['detalle_pago']);
        ...
        if (!empty($pedidoData['data'])) {
            ...
            if ($pedidoData['detalle_pago']['data']['factura']) {
                $mensajeCompraUsuario .= '<p style="float:left><b>' . $_SESSION["lang-txt"]["api"]["email"]["factura_cuit"] . ' </b>' . $pedidoData['detalle_pago']['data']['documento'] . '</p>';
            }

            $mensajeCompraUsuario .= '<h6 style="width:100%;float:left">' . $_SESSION["lang-txt"]["api"]["email"]["metodo_pago"] . ' ' . mb_strtoupper($pedidoData['data']["pago_titulo"]) . '</h6>';
            if (isset($pedidoData['data']['link_pago']) && $estado['data']['estado'] != '2') {
                $mensajeCompraUsuario .= '<h6>' . $_SESSION["lang-txt"]["api"]["email"]["link_pago"] . ' <a href="' . $pedidoData['data']['link_pago'] . '"> ' . $_SESSION["lang-txt"]["api"]["email"]["click_aqui"] . '</a></h6>';
            }
            ...
            if ($enviar_Admin) {
                ...
                if ($pedidoData['detalle_pago']['data']['factura']) {
                    $mensajeCompra .= '<p><b>Factura A al CUIT: </b>' . $pedidoData['detalle_pago']['data']['documento'] . '</p>';
                }
                $mensajeCompra .= '<br/><hr/>';
                $mensajeCompra .= '<h5 style="width:100%;float:left"><b>MÉTODO DE PAGO ELEGIDO:</b>' . mb_strtoupper($pedidoData['data']["pago_titulo"]) . '</h5>';
