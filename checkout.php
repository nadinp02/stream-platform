...
if($op != 'detail')$checkout->checkTypeCheckout($op);
...
?>
    <script>
        const checkoutFinal = async () => {
            await saveToPdf("<?= $_SESSION["cod_pedido"] ?>");
            await sendBuyTimer('<?= URL ?>', '<?= $_SESSION['cod_pedido'] ?>', '<?= $pedidoData['data']['estado'] ?>', '<?= $estadoData['data']['enviar'] ?>', true);
        }
        checkoutFinal();
    </script>
<?php
